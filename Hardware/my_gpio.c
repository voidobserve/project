...
{
    P2_MD1 &= ~(GPIO_P24_MODE_SEL(0x1)); // 对应的寄存器先清零
    P2_MD1 |= GPIO_P24_MODE_SEL(0x1);    // 配置为输出模式

    FOUT_S24 = GPIO_FOUT_AF_FUNC; // 选择AF功能输出

    // P2_PU |= GPIO_P24_PULL_UP(0x01); // 内部的上拉电阻
    // P2_PD |= GPIO_P24_PULL_PD(0x01); // 内部的下拉电阻
    // 配置完上下拉电阻后，这个引脚就相当于浮空状态，同时能够避免一些电路的干扰
...
{
    P2_MD1 &= ~(GPIO_P25_MODE_SEL(0x1)); // 对应的寄存器先清零
    P2_MD1 |= GPIO_P25_MODE_SEL(0x1);    // 配置为输出模式

    FOUT_S25 = GPIO_FOUT_AF_FUNC; // 选择AF功能输出

    // P2_PU |= GPIO_P25_PULL_UP(0x01); // 内部的上拉电阻
    // P2_PD |= GPIO_P25_PULL_PD(0x01); // 内部的下拉电阻
    // 配置完上下拉电阻后，这个引脚就相当于浮空状态，同时能够避免一些电路的干扰
