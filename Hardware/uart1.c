...
void uart1_config(void)
{
    ...
    P1_MD1 |= GPIO_P14_MODE_SEL(0x1); // 输出模式
    FOUT_S14 |= GPIO_FOUT_UART1_TX;   // 配置为UART1_TX

    // // P15为发送引脚
    // P1_MD1 &= ~GPIO_P15_MODE_SEL(0x3) ;
    // P1_MD1 |= GPIO_P15_MODE_SEL(0x1); // 输出模式
    // FOUT_S15 |= GPIO_FOUT_UART1_TX;   // 配置P15为UART1_TX
...
// // UART1中断服务函数（如果使用DMA，DMA接收满才会触发该中断）
